...
public class A_Star_H_One {
   public int space;
    ...
    public A_Star_H_One(int[] puzzle) {
        this.puzzle = puzzle;
        stack = new Stack<>();
    ...
    public void solve() {
        ...
        Node2 currentNode;
        fnMax = new NodeCostFunction().findcost(node);
...
//                   PlaySolution();
    ...
    public void removeHigherHeuristicFromStack() {
        ...
        for (int i = 0; i < stack.size(); i++) {
            ...
            if (new NodeCostFunction().findcost(n) < minHeuristic) {
                minHeuristic = nodeCostCalculate.findcost(n);
            } else if (nodeCostCalculate.findcost(n) > minHeuristic) {
                stack.remove(i);
            }
    ...
    public List GenerateStateNodes(Node2 node) {
        ...
        if (zeroPOS == 8) {
            ...
            stateLEFT.setCost(nodeCostCalculate.findcost(stateLEFT.getPuzzle()) + 1);
            stateUP.setCost(nodeCostCalculate.findcost(stateUP.getPuzzle()) + 1);
            ...
            stateLEFT.setCost(nodeCostCalculate.findcost(stateLEFT.getPuzzle()) + 1);
            stateRIGHT.setCost(nodeCostCalculate.findcost(stateRIGHT.getPuzzle()) + 1);
            stateUP.setCost(nodeCostCalculate.findcost(stateUP.getPuzzle()) + 1);
            ...
            stateLEFT.setCost(nodeCostCalculate.findcost(stateLEFT.getPuzzle()) + 1);
            stateUP.setCost(nodeCostCalculate.findcost(stateUP.getPuzzle()) + 1);
            stateDOWN.setCost(nodeCostCalculate.findcost(stateDOWN.getPuzzle()) + 1);
            list.add(stateLEFT);
            list.add(stateUP);
            list.add(stateDOWN);
