 ...
 * @author Chetan
 */
public class Greedy_BFS_H_One_NumOfTiles {
public int space;
    int[] puzzle;
    ...
    NodeCostFunction nodeCostCalculate;
    long startTime;
    long stopTime;
    public String time;

    public Greedy_BFS_H_One_NumOfTiles(int[] puzzle) {
        this.puzzle = puzzle;
        stack = new Stack<>();
        helper = new HelperMethods();
        map = new HashMap<>();
        nodeCostCalculate = new NodeCostFunction();
    ...
    public void solve() {
        ...
        Node2 currentNode;
        fnMax = new NodeCostFunction().findGreedyCostH1(node);
...
//                   PlaySolution();
    ...
    /**
     * Method to remove the higher heuristic from the stack
     */
    public void removeHigherHeuristicFromStack() {
       
        
        
        int minHeuristic = fnMax;
        for (int i = 0; i < stack.size(); i++) {
            Node2 n = stack.elementAt(i);
            System.out.println("Node " + n);

            if (new NodeCostFunction().findGreedyCostH1(n) < minHeuristic) {
                minHeuristic = nodeCostCalculate.findGreedyCostH1(n);
            } else if (nodeCostCalculate.findGreedyCostH1(n) > minHeuristic) {
                stack.remove(i);
            }
    ...
    public List GenerateStateNodes(Node2 node) {
        ...
        if (zeroPOS == 8) {
            ...
            stateLEFT.setCost(nodeCostCalculate.findGreedyCostH1(stateLEFT.getPuzzle()) + 1);
            stateUP.setCost(nodeCostCalculate.findGreedyCostH1(stateUP.getPuzzle()) + 1);
            ...
            stateLEFT.setCost(nodeCostCalculate.findGreedyCostH1(stateLEFT.getPuzzle()) + 1);
            stateRIGHT.setCost(nodeCostCalculate.findGreedyCostH1(stateRIGHT.getPuzzle()) + 1);
            stateUP.setCost(nodeCostCalculate.findGreedyCostH1(stateUP.getPuzzle()) + 1);
            ...
            stateLEFT.setCost(nodeCostCalculate.findGreedyCostH1(stateLEFT.getPuzzle()) + 1);
            stateUP.setCost(nodeCostCalculate.findGreedyCostH1(stateUP.getPuzzle()) + 1);
            stateDOWN.setCost(nodeCostCalculate.findGreedyCostH1(stateDOWN.getPuzzle()) + 1);
            list.add(stateLEFT);
            list.add(stateUP);
            list.add(stateDOWN);
